...
(defun general-search (problem queuing-fn)
  ...
  (let ((nodes (make-initial-queue problem queuing-fn))
	node)
    (loop (if (empty-queue? nodes)
	    (RETURN nil))
	  (setq node (remove-front nodes))
	  (if (goal-test problem (node-state node))
	    (progn (display-expand problem node 0 (queue-size nodes))
		   (RETURN node)))
	  (let ((successor-nodes (expand node problem)))
	    (funcall queuing-fn nodes successor-nodes)
	    (display-expand problem node (length successor-nodes) (queue-size nodes))))))
...
(defun depth-limited-search (problem &optional (limit most-positive-single-float)
