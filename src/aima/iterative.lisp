...
(defun hill-climbing-search (problem 
  ...
  (let ((current (create-start-node problem))
	next)
    (loop
      (display-expand problem current 0 0)
	 ...
	 (return (values current (goal-test problem (node-state current)))))
       (setf current next)))))
...
(defun simulated-annealing-search (problem &optional
  ...
  (let* ((current (create-start-node problem))
	 ...
	 (best current)
	 next temp delta)
    (loop for time from 1 upto infinity do
      (display-expand problem current 0 0)
	  (setf temp (funcall schedule time))
	  (when (or (= temp 0) (null successors))
	    (RETURN (values best (goal-test problem (node-state best)))))
...
(defun hill-climbing-until-flat-n-times-search (problem &optional (n 4000))
...
(defun make-exp-schedule (&key (k 20) (lambda 0.005) (limit 10000))
