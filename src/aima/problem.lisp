...
(defgeneric solve (problem &optional algorithm show-solution))
(defgeneric successors (problem state))
(defgeneric goal-test (problem state))
(defgeneric h-cost (problem state))
(defgeneric edge-cost (problem node action state))
(defgeneric print-structure (node stream))
(defgeneric display-expand (problem node count total))
...
;(defmethod successors :before ((problem problem) state)
;  (incf (problem-num-successor-calls problem)))
...
(defmethod solve ((problem problem) &optional (algorithm 'A*-search)
		  (show-solution t))
  "Print a list of actions that will solve the problem (if possible).
  Return the node that solves the problem, or nil."
  (write-log "~2a ~3a ~3a ~3a ~6a ~9a ~3a ~26a ~3a ~10a ~10a~%"
	     "d" "f" "g" "h" "c" "s" "b" "bvms" "xhs" "rem" "w")
...
(defmethod print-object ((node node) stream) 
  (format stream "#<NODE depth: ~a f(~D) = g(~D) + h(~D) state:~A action:~A>"
	  (node-depth node)
	  (node-f-cost node)
	  (node-g-cost node)
	  (node-h-cost node)
...
(defmethod display-expand ((problem problem) node count total)
  "Possibly print information when a node is expanded."
  (write-log "~2d ~3d ~3d ~3d ~a ~a~%"
	  (node-depth node)
	  (node-f-cost node)
	  (node-g-cost node)
	  (node-h-cost node)
