...
(defun tree-sma (problem &optional (memory-size 2000000)
			 &aux n
  ...
  (loop 
   ...
   (when (goal-test problem (node-state n))
     (return n))
   (when (= (node-f-cost n) most-positive-single-float) (return nil))
...
(defun tree-backup-f-cost (node q &optional (was-open? nil) 
				       ...
				       (least most-positive-single-float)) 
